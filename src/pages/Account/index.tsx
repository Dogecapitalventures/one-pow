...
const Accountpage = () => {
  ...
  const [balanceOwt, setBalanceOwt] = useState<string>("0");
...
const getData = async () => {
  ...
  let owtbl = "0";
  if(account){
    owtbl = await contract?.balanceOf(account as string)    
  } else {
    owtbl = "0";
  }
  ...
  accountTableData[1].value = ((parseFloat(owtbl)/100000)*0.02355).toFixed(2).toString()+" OWT";
  accountTableData[2].value = "$"+(parseFloat(priceUsd.data.pair.priceUsd)*((parseFloat(owtbl)/100000)*0.02355)).toFixed(2).toString();
  accountTableData[4].value = "$"+(parseFloat(priceUsd.data.pair.priceUsd)*((parseFloat(owtbl)/100000)*2.28)).toFixed(2).toString();
  accountTableData[5].value = (((parseFloat(priceUsd.data.pair.priceUsd)*(parseFloat(owtbl)/100000))*11.96)/100).toFixed(2).toString()+"%";
  accountTableData[6].value = "$"+(parseFloat(priceUsd.data.pair.priceUsd)*((parseFloat(owtbl)/100000)*11.96)).toFixed(2).toString();
  
  
  let owtblUsd = parseFloat(priceUsd.data.pair.priceUsd)*(parseFloat(owtbl)/100000)
  setBalanceOwt((parseFloat(owtbl)/100000).toFixed(2).toString());
  setBalanceUsd(owtblUsd.toFixed(2).toString());
      ...
      <ToastContainer />
        <div className="col-lg-4 col-md-12 d-flex justify-content-end flex-column left align-items-center pb-3">
          <div className="card parallel-border-lr ">
            ...
            <p className="title-body1">{formatter.format(parseFloat(balanceOwt)) || 0} OWT</p>
          </div>
        </div>
