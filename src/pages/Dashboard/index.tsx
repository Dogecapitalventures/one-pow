...
import OOWT from "../../config/contract/abi/OOWT.json";
...
const Dashboard = () => {
  ...
  const [owtPrice, setOwtPrice] = useState<string>();
...
// const signer = provider.getSigner()
  ...
  let priceUsd = await axios.get("https://api.dexscreener.com/latest/dex/pairs/ethereumpow/0xA82e74921eC3C26Ee2E7f6c2FD6154D44Db71213")
  if(!priceUsd.data.pair){
    setOwtPrice("0")
  }else {
    setOwtPrice(priceUsd.data.pair.priceUsd)
  }
  let owtbl = await contract?.balanceOf(REACT_APP_TREASURY_WALLET as string)
  let wdog = await provider?.getBalance(REACT_APP_TREASURY_WALLET as string)
  let poolVOwt = await contract?.balanceOf(REACT_APP_POOL_VALUE as string)
  let poolWdog = await provider?.getBalance(REACT_APP_POOL_VALUE as string)
  let insOwt = await contract?.balanceOf(REACT_APP_DCP_INSURANCE as string)
  let insWdog = await provider?.getBalance(REACT_APP_DCP_INSURANCE as string)
  let fireOwt = await contract?.balanceOf(REACT_APP_FIREPIT as string)
  ...
  let cts = parseInt(ts)-(parseInt(fireOwt)/100000);
  // alert(cts)
  ...
  // setFirePitVal((parseFloat(owtbl)/100000).toString());
  // setBalance(ts.toString());
  // bl = await provider.getBalance("0x24B2098605289fFB826115EA0F9f8fC1ab5b816F");
  // setBalance(bl.toString());
  let owtblU = parseFloat(priceUsd.data.pair.priceUsd)*(parseFloat(owtbl)/100000)
  let fireOwtblU = parseFloat(priceUsd.data.pair.priceUsd)*(parseFloat(fireOwt)/100000)
  setFirePitVal(fireOwtblU.toString());
  let ethw = parseFloat(priceUsd.data.pair.priceUsd)*((parseFloat(wdog)/1000000000000000000) as any)
  setTreasurayAsset((owtblU+ethw).toString())
  let poolOwtblU = parseFloat(priceUsd.data.pair.priceUsd)*(parseFloat(poolVOwt)/100000)
  let poolWdoge = parseFloat(priceUsd.data.pair.priceUsd)*((parseFloat(poolWdog)/1000000000000000000) as any)
  setPoolVal((poolOwtblU+poolWdoge).toString())
  let insOwtblU = parseFloat(priceUsd.data.pair.priceUsd)*(parseFloat(insOwt)/100000)
  let insWdoge = parseFloat(priceUsd.data.pair.priceUsd)*((parseFloat(insWdog)/1000000000000000000) as any)
  setInsFund((insOwtblU+insWdoge).toString())
      ...
      <ToastContainer />
        <div className="col-lg-4 col-md-12 d-flex justify-content-end flex-column left align-items-center">
          ...
          <div className="card parallel-border-lr mx-2">
            ...
            <div className="card-value">100,0000 OWT</div>
          </div>
          <div className="card parallel-border-lr mx-2">
            <div className="card-title-regular">Circulating Supply</div>
            <div className="card-value">70,0000</div>
          </div>
        </div>
        <div className="col-lg-4 col-md-12 middle d-flex flex-column justify-content-center align-items-center">
          <div className="d-flex align-items-center justify-content-center flex-column">
            <div className="btn--outline card parallel-border-lr">
              OWT PRICE
            <div className="card-value">${owtPrice}</div>
            </div>
